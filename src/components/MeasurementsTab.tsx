...
import { Sensor, Measurement, SensorType, SENSOR_TYPES } from '../types';
...
export default function MeasurementsTab({ sensors, canWrite }: MeasurementsTabProps) {
  ...
  const loadMeasurements = async () => {
    try {
      const data = await MeasurementService.getMeasurements(sensors);
  ...
  // Ensure each sensor keeps a stable color across charts and renders
  const colorForSensorId = (id: number) => colors[Math.abs(id) % colors.length];

  const filteredSensors: Sensor[] = useMemo(() => {
    // Show all sensors when no filter is applied
    if (filterSensorType === 'all') return sensors;
    // Keep a soft cap for type-specific views to avoid overcrowding
  ...
  const temperatureSeries: Series[] = useMemo(() => {
    ...
    for (const m of measurements) {
      ...
      // Use the stable Date object from the service instead of re-parsing a localized string
        ...
        ListHeaderComponent={
          <View style={styles.chartsContainer}>
            ...
            <View style={styles.filterRow}>
              {(['all', ...SENSOR_TYPES] as const).map((t, idx) => (
                <TouchableOpacity
                  key={String(t)}
