...
import { Sensor, SENSOR_TYPES } from '../types';
...
export default function SensorsTab({ sensors, onSensorsChange, canWrite }: SensorsTabProps) {
  ...
  const handleAdd = async () => {
    ...
    setLoading(true);
    try {
      const newSensor = await SensorService.createSensor(formData);
  ...
  const handleUpdate = async () => {
    ...
    setLoading(true);
    try {
      const updated = await SensorService.updateSensor(editingSensor.id, formData);
